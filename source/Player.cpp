...
{	
	...
	case PLAYER_STATES_RUN: 
		...
		// exit condition:
			 if (Game::sInput->IsKeyReleased(CONTROLS_TRIGGER_PUNCH))	{ Punch(); }
		else if (Game::sInput->IsKeyDown(CONTROLS_MOVE_EAST))			{ SetFacing(EAST); }
		else if (Game::sInput->IsKeyDown(CONTROLS_MOVE_NORTH))			{ SetFacing(NORTH); }
		else if (Game::sInput->IsKeyDown(CONTROLS_MOVE_WEST))			{ SetFacing(WEST);}
		else if (Game::sInput->IsKeyDown(CONTROLS_MOVE_SOUTH))			{ SetFacing(SOUTH); }
		else SetBehavior(PLAYER_STATES_IDLE);
