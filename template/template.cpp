...
int main()
{
...
#ifdef FULLSCREEN
	window = glfwCreateWindow( SCRWIDTH, SCRHEIGHT, "Metal Gear 1 MSX2 - Block A Retake - Ayser Hartgring", glfwGetPrimaryMonitor(), 0 );
#else
	window = glfwCreateWindow( SCRWIDTH, SCRHEIGHT, "Metal Gear 1 MSX2 - Block A Retake - Ayser Hartgring", 0, 0 ); 
#endif
	...
	InitRenderTarget(NATIVE_SCREEN_WIDTH, NATIVE_SCREEN_HEIGHT);
	Surface8::InitPalette(); 
	Surface8* screen8	= new Surface8(NATIVE_SCREEN_WIDTH, NATIVE_SCREEN_HEIGHT); 
	Surface* screen		= new Surface(NATIVE_SCREEN_WIDTH, NATIVE_SCREEN_HEIGHT);
	app = new Game();
	app->screen8 = screen8;    
	app->screen = screen;
	app->Init();
	...
	while (!glfwWindowShouldClose( window ))
	{
		...
		if (frameNr++ > 1)
		{
			if (app->screen8 && app->screen) app->screen8->CopyTo(app->screen);   
